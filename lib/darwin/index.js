...
function parseDisplaysOutput (json) {
  let displayinfos = []
  json.SPDisplaysDataType.forEach((spdisplay) => {
    if ("spdisplays_ndrvs" in spdisplay) {
      spdisplay.spdisplays_ndrvs.forEach((key) => {
        const temp = {
          name: key["_name"],
          primary: key["spdisplays_main"] === "spdisplays_yes",
          displayId: parseInt(key["_spdisplays_displayID"], 16),
        };
  ...
  return movePrimaryToHead(addId(displayinfos));
}
  ...
  return new Promise((resolve, reject) => {
    exec(
      ...
      (err, stdout) => {
        ...
        resolve(parseDisplaysOutput(JSON.parse(stdout)))
      })
  })
}
